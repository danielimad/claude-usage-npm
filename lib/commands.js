...
const REPO_RAW   = "https://raw.githubusercontent.com/danielimad/claude-usage-bar/main";
...
async function install() {
  ...
  try {
    const appCode = await fetchFile(`${REPO_RAW}/claude_usage_bar/__main__.py`);
    fs.writeFileSync(APP_PY, appCode);
    ok("App downloaded");
  } catch (e) {
    err(`Failed to download app: ${e.message}`);
    err("Check your internet connection and that the GitHub repo is public");
    process.exit(1);
  }
